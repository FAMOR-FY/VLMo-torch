...
from . import heads, objectives, vlmo_utils
...
class VLMo(nn.Module):
    def __init__(self, config):
        ...
        if self.config["load_path"] != "" and self.config["test_only"]:
            logger.info("Load ckpt from: {}".format(self.config["load_path"]))
    ...
    def load_pretrained_weight(self):
        if self.config["load_path"] != "" and not self.config["test_only"]:
            config = self.config
            ckpt = torch.load(self.config["load_path"], map_location="cpu")
            logger.info("Load ckpt from: {}".format(self.config["load_path"]))
            ...
            for key in state_dict:
                var = state_dict[key]
                logger.info("%s = %s" % (key, str(var.size())))

            logger.info(config["loss_names"])
            if config["loss_names"]["textmlm"] > 0:
                logger.info("convert to textpt")
            ...
            if "text_embeddings.position_embeddings.weight" in state_dict and state_dict["text_embeddings.position_embeddings.weight"].size(0) != max_text_len:
                ...
                logger.info("text position_embeddings size: {}".format(state_dict["text_embeddings.position_embeddings.weight"].size()))
    ...
    def build_relative_position_embed(self, config):
        ...
        # logger.info("min_distance: {}".format(min_distance))
